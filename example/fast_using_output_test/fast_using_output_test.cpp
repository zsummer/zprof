...
int main(int argc, char* argv[])
{
    ...
    PROF_OUTPUT_MULTI_COUNT_CPU("default output", 1000, cost.cycles());



    PROF_SET_OUTPUT(&OutputPrint);
    PROF_OUTPUT_MULTI_COUNT_CPU("print output", 1000, cost.cycles());

    PROF_SET_OUTPUT(&OutputLog);
    PROF_OUTPUT_MULTI_COUNT_CPU("log output", 1000, cost.cycles());
